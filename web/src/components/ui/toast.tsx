...
function Toast({ className, ...props }: ToastPrimitive.Root.Props) {
  return (
    <ToastPrimitive.Root
      data-slot="toast"
      className={cn(
        "group/toast pointer-events-auto absolute right-0 bottom-0 z-[calc(1000-var(--toast-index))] w-full origin-bottom rounded-surface border border-hairline-strong bg-card text-card-foreground shadow-popover will-change-transform outline-none select-none focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/50",
...
function ToastContent({ className, ...props }: ToastPrimitive.Content.Props) {
  return (
    <ToastPrimitive.Content
      ...
      className={cn(
        "flex h-full items-center gap-2.5 overflow-hidden px-3.5 py-3 transition-opacity duration-base ease-out data-behind:opacity-0 data-expanded:opacity-100",
        className
      )}
      {...props}
    />
...
function ToastIcon({ type }: { type: string | undefined }) {
  if (type === "loading") {
    return (
      <span data-slot="toast-icon" className="shrink-0 text-mute [&_svg]:size-3.5">
        <Loader2Icon className="animate-spin" aria-hidden="true" />
      </span>
    )
  }

  const dot = type ? TOAST_DOT[type] : undefined
  if (!dot) {
    return null
  }
  ...
  return (
    <span
      ...
      className={cn("size-[7px] shrink-0 rounded-pill", dot)}
      aria-hidden="true"
    />
  )
}
