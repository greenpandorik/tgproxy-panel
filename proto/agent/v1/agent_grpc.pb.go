...
const (
	AgentGateway_Session_FullMethodName = "/agent.v1.AgentGateway/Session"
)
...
func (c *agentGatewayClient) Session(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[Envelope, Envelope], error) {
...
type AgentGateway_SessionClient = grpc.BidiStreamingClient[Envelope, Envelope]
...
func (UnimplementedAgentGatewayServer) Session(grpc.BidiStreamingServer[Envelope, Envelope]) error {
...
func _AgentGateway_Session_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(AgentGatewayServer).Session(&grpc.GenericServerStream[Envelope, Envelope]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AgentGateway_SessionServer = grpc.BidiStreamingServer[Envelope, Envelope]
