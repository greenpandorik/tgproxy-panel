...
Everything the [README](../README.md) does not need to say up front: how the panel is
installed by hand, how a node is built, how the two talk, and what every screen, setting
and environment variable does. For a walkthrough with screenshots, start with the
[setup guide](setup.en.md) instead.
...
- Telegram reachability per node: latency to every Telegram data centre as telemt measures it, the health of the route and a connection counter, with a history chart; the nodes list and the dashboard carry the overall latency.
...
If an admin loses both their authenticator and their recovery codes, see [Two-factor authentication lockout](runbook.md#two-factor-authentication-lockout). The escape hatch is a host-side CLI command, not a panel route:
...
Restore is a destructive CLI command, not a panel route (`pg_restore --clean --if-exists` drops every existing object first): see [Backups and restore](runbook.md#backups-and-restore) in the runbook for the full stop-panel/restore/start-panel procedure.
...
It reads the new key from `MASTER_KEY_NEW` (32 random bytes, base64) and refuses to run if that equals the current `MASTER_KEY`, since rotating onto the same key material would just burn a version number. It never prints key material, only row counts and env-variable placeholders. Full procedure, including what to do with the old key afterwards, in [Master key rotation](runbook.md#master-key-rotation).

## Grafana

A ready-made dashboard (node/key counts, live sessions/streams) lives at [`deploy/grafana/tgwp-panel.json`](../deploy/grafana/tgwp-panel.json), built on top of the `/metrics` Prometheus endpoint described under [Monitoring](#monitoring) above. See [`docs/monitoring.md`](monitoring.md) for importing it and the full metric reference.
