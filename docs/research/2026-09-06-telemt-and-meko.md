...
Обновление 2026-09-07: закреплённая версия поднята до **3.5.6** (релиз 2026-09-06). Релиз про WEB-транспорт: восстановление моста после обрыва (bounded, с дедлайном и одним владельцем), управление жизненным циклом WEB без перезапуска процесса (`POST /v1/runtime/web/lifecycle/{pause,drain,resume}`), выбор поведения при переполнении соединений (`web.http_connection_capacity_action`: drop/wait/respond), decoy fast-track (`off`/`shadow`/`enforce`), расширенные `telemt_web_*` метрики фиксированной кардинальности. Изменения API только дополняющие: `/v1/users`, `/v1/config`, `/v1/system/reload`, `/v1/health/ready`, `/v1/system/info` не тронуты, конфиг, который генерирует наш агент, остаётся валидным. Смена семантики: `reconnect_grace_secs` теперь считает подтверждённую неактивность клиента, пустые poll-ы и трафик только от бэкенда сессию больше не продлевают. Новые параметры имеют обратно совместимые значения по умолчанию, мы их не задаём.

Обновление 2026-09-08: пин поднят до **3.5.7** (релиз 2026-09-08). Два исправления, оба про WEB: WEB-прокси на Telegram для macOS и восстановление WebSocket после рестарта telemt. Ни конфиг, ни API, ни документация не менялись, чистый багфикс.
